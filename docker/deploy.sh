...
FLOWISE_ADMIN_EMAIL=$(_vault flowise_admin_email);     FLOWISE_ADMIN_EMAIL="${FLOWISE_ADMIN_EMAIL:-flowise-admin@flowise.local}"
FLOWISE_ADMIN_PASSWORD=$(_vault flowise_admin_password)
if [[ -z "${FLOWISE_ADMIN_PASSWORD}" ]]; then
    die "FLOWISE_ADMIN_PASSWORD not found in vault.yml. Please regenerate vault.yml by running: bash docker/scripts/generate-vault-secrets.sh"
fi
FLOWISE_API_KEY=$(_vault flowise_api_key)               # may be empty — set after first run
...
for p in "${PROFILES[@]:-}"; do
    case "${p}" in
        flowise) echo "  Flowise Admin    : http://localhost:3000" ;;
    esac
done
